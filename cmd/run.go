...
func runCommandHandler(cmd *cobra.Command, args []string) {
	...
	if tapDeviceName != "" {
		...
		if err != nil {
			panic("Not able to check tap exists")
		}

		if !tapExists {
			_, err := network.AddTap(tapDeviceName)
			if err != nil {
				panic("Not able to create tap")
			}
		}
		...
		if bridged {
			...
			if err != nil {
				panic("Not able to check if bridge exists")
			}

			if !bridgeExists {
				_, err := network.AddBridge(bridgeName)
				if err != nil {
					panic("Not able to create bridge")
				}
			}
			...
			if err != nil {
				panic("Not able to check if tap is in the bridge")
			}

			if !isTapInTheBridge {
				_, err := network.AddTapToBridge(bridgeName, tapDeviceName)
				if err != nil {
					panic("Not able to add tap to bridge")
				}
			}

			isBridgeUp, err := network.IsNIUp(bridgeName)
			if err != nil {
				panic("Not able to check if bridge is up")
			}

			if !isBridgeUp {
				_, err := network.TurnNIUp(bridgeName)
				if err != nil {
					panic("Not able to turn bridge up")
				}
			}
		}
		...
		if err != nil {
			panic("Not able to check if tap is up")
		}

		if !isTapUp {
			_, err = network.TurnNIUp(tapDeviceName)
			if err != nil {
				panic("Not able to turn tap up")
			}
		}
	}
	...
	if tapDeviceName != "" {
		...
		if err != nil {
			panic("Not able to turn tap down")
		}

		if bridged {
			bridgeInterfaces, err := network.GetBridgeInterfacesNames(bridgeName)
			if err != nil {
				panic("Not able to get network interfaces in bridge")
			}
			...
			for _, bi := range bridgeInterfaces {
				...
				if err != nil {
					panic("Not able to get interface state")
				}
			...
			if !someBridgeInterfaceIsUp {
				...
				if err != nil {
					panic("Not able to turn bridge down")
				}

				_, err = network.FlushIPFromNI(bridgeName)
				if err != nil {
					panic("Not able to flush IPs from bridge")
				}
			}

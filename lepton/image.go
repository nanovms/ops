...
// userImage is the user supplied ELF binary.
// bootImage is the final bootable Image.
...
func DownloadImages(w io.Writer) error {
	...
	if _, err = os.Stat("staging/boot"); os.IsNotExist(err) {
	  if err = downloadFile("staging/boot", fmt.Sprintf(bucketBaseUrl, "boot"), 
	  						w); err != nil {
	  	return err
	  }
	}
	...
	if _, err = os.Stat("staging/stage3"); os.IsNotExist(err) {
	  if err = downloadFile("staging/stage3", fmt.Sprintf(bucketBaseUrl, "stage3"), 
							w); err != nil {
	  	return err
	  }
	}
